		...
		reportStream
			...
			.on("data", (report_record) => {
				...
				if (unblended_cost || blended_cost) {
					...
					const billing_period = `${moment(billing_period_start_date).format("MM/DD/YYYY")}-${moment(billing_period_end_date).format("MM/DD/YYYY")}`;
					...
					cost_report[billing_period].service[aws_service] = cost_report[billing_period].service[aws_service]
						? cost_report[billing_period].service[aws_service]
						: {};
					cost_report[billing_period].service[aws_service].blended_cost = cost_report[billing_period].service[aws_service].blended_cost
						? (cost_report[billing_period].service[aws_service].blended_cost += blended_cost)
						: blended_cost;
					cost_report[billing_period].service[aws_service].unblended_cost = cost_report[billing_period].service[aws_service].unblended_cost
						? (cost_report[billing_period].service[aws_service].unblended_cost += unblended_cost)
						: unblended_cost;
					...
					if (region !== "") {
						...
						cost_report[billing_period].region[region].blended_cost = cost_report[billing_period].region[region].blended_cost
							? (cost_report[billing_period].region[region].blended_cost += blended_cost)
							: blended_cost;
						cost_report[billing_period].region[region].unblended_cost = cost_report[billing_period].region[region].unblended_cost
							? (cost_report[billing_period].region[region].unblended_cost += unblended_cost)
							: unblended_cost;
					}
					...
					cost_report[billing_period].account[account_id] = cost_report[billing_period].account[account_id]
						? cost_report[billing_period].account[account_id]
						: {};
					cost_report[billing_period].account[account_id].blended_cost = cost_report[billing_period].account[account_id].blended_cost
						? (cost_report[billing_period].account[account_id].blended_cost += blended_cost)
						: blended_cost;
					cost_report[billing_period].account[account_id].unblended_cost = cost_report[billing_period].account[account_id].unblended_cost
						? (cost_report[billing_period].account[account_id].unblended_cost += unblended_cost)
						: unblended_cost;
				}
			})
